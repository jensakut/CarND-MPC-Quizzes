 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
	  ...
	  //only the past inputs are needed
	  ...
	  //v_1 = v_0 + a * dt 
	  fg[1 + v_start + t] = v1 + a0 * dt; 
	  //CTE_1 = f(x0) - y0 + v0 / Lf *delta0 *dt 
	  fg[1 + cte_start + t] = f_x0 - y0 + v0 / Lf * delta0 * dt;
	  //epsi_1 = psi1 - espi_desired
	  //epsi_desired = atan(f'(x0)) = atan(coeffs[1])
	  fg[1 + epsi_start + t] = fg[1 + psi_start + t] - CppAD::atan(coeffs[1]);
	                          // psi1                   atan(f'(x0))
							  */
	  fg[1 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);
